...
check_file ".antigravity/hooks/api-doc-sync-check.json"
check_file ".antigravity/hooks/layer-constraint-check.json"
check_file ".antigravity/hooks/test-coverage-reminder.json"
check_file ".antigravity/hooks/maven-profile-check.json"
echo ""
...
STEERING_COUNT=0
for f in .antigravity/steering/*.md; do
  if [ -f "$f" ]; then
...
if [ $ERRORS -eq 0 ]; then
  ...
  echo "新建 Spring Boot 子项目后，在 Antigravity 中打开工作区根目录即可开始受监管的开发。"
else
