...
本手册定义 Antigravity 在 Spring Boot Harness 工作区中的操作规范，包括每个研发阶段的具体行为、交付摘要格式和常用命令速查表。
...
1. **接收需求描述**：当开发者描述新功能时，Antigravity 首先确认是否已有对应的需求文档
   ```
...
每次完成一个功能或阶段后，Antigravity 必须输出以下格式的交付摘要：
...
## 与 Antigravity 协作的最佳实践

### 1. 明确描述需求背景

与 Antigravity 协作时，提供充分的上下文信息能显著提升代码质量：
...
Antigravity 的 Hook 会在关键节点自动提示，不要忽略这些提示：
