...
本功能旨在构建一套面向 Spring Boot 项目的 Antigravity Harness 工程管理模板。该模板通过 Antigravity IDE 的原生机制（steering 文件、spec 规范、hooks 自动化）将工程规范、编码标准、测试策略和 CI/CD 配置内嵌到 AI 辅助开发流程中，确保 Antigravity 在每次交付时都能遵循高质量标准，实现"规范即代码、门禁即流程"的研发基线。

**核心定位**：Harness 模板作为工作区根目录的"监管层"，当开发者在同一工作区内创建新的 Spring Boot 子项目时，Harness 的 steering 文件、hooks 和规范文档自动对该子项目生效，Antigravity 在整个开发生命周期（需求 → 设计 → 编码 → 测试 → 发布）中均受 Harness 约束，无需额外配置。
...
- **Harness_Template**：本模板整体，包含所有 Antigravity 配置文件、工程规范文档和 CI/CD 配置
- **Antigravity_IDE**：目标 AI 辅助开发工具，通过 steering 文件、spec 规范和 hooks 驱动开发流程
- **Steering_File**：`.antigravity/steering/` 目录下的 Markdown 文件，Antigravity 在每次对话中自动加载，用于约束 AI 行为
- **Spec**：`.antigravity/specs/` 目录下的规范文档集合（requirements、design、tasks），驱动功能开发
- **Hook**：`.antigravity/hooks/` 目录下的自动化触发器，在特定 IDE 事件发生时执行检查或提示
...
- **Workspace_Root**：Antigravity 工作区根目录，Harness 模板部署于此，对同工作区内所有子项目生效
...
### 需求 1：Antigravity Steering 文件体系

**用户故事：** 作为使用 Antigravity IDE 的开发者，我希望模板提供完整的 steering 文件集，以便 Antigravity 在每次对话中自动遵循 Spring Boot 工程规范，无需手动重复说明约束。

#### 验收标准

1. THE Harness_Template SHALL 在 `.antigravity/steering/` 目录下提供以下 steering 文件：Java 工程规范、测试与质量规范、API 文档同步规范、AI 协作协议。
2. WHEN Antigravity_IDE 加载项目时，THE Steering_File SHALL 自动约束 Antigravity 遵循四层架构（controller → service → domain ← repository）。
3. THE Steering_File SHALL 明确禁止跨层直接依赖（如 controller 直接调用 repository）。
4. WHEN Antigravity 生成 Java 代码时，THE Steering_File SHALL 要求代码符合 Checkstyle 规则，包含必要的 Javadoc 注释。
...
**用户故事：** 作为开发者，我希望模板定义清晰的 Spring Boot 项目目录结构和包结构规范，以便 Antigravity 生成的代码始终符合团队约定的工程布局。
...
3. WHEN Antigravity 创建新的业务类时，THE Steering_File SHALL 要求将类放置在与其职责对应的正确包层级中。
...
6. IF Antigravity 生成的代码违反分层约束，THEN THE Hook SHALL 触发警告提示，要求开发者确认或修正。
...
**用户故事：** 作为开发者，我希望模板提供完整的测试规范和测试骨架代码，以便 Antigravity 生成的测试代码符合分层测试策略，覆盖单元测试、集成测试和 API 测试。
...
3. WHEN Antigravity 生成业务代码时，THE Steering_File SHALL 要求同步生成对应的测试类，测试类与被测类保持相同的包路径。
...
### 需求 7：Antigravity Hooks 自动化

**用户故事：** 作为开发者，我希望模板提供预配置的 Antigravity Hooks，以便在关键开发节点自动触发质量检查和文档同步提醒，减少人工遗漏。

#### 验收标准

1. THE Harness_Template SHALL 在 `.antigravity/hooks/` 目录下提供以下 Hook 配置：API 文档同步检查 Hook、代码分层约束检查 Hook、测试覆盖提醒 Hook。
...
**用户故事：** 作为开发者，当我在当前工作区创建新的 Spring Boot 子项目时，我希望 Harness 模板能自动对该子项目的整个研发生命周期进行监管，Antigravity 在每个阶段都能获得明确的规范指引，无需额外配置。

#### 验收标准

1. THE Harness_Template SHALL 在 `.antigravity/steering/` 中提供 `project-lifecycle.md` steering 文件，定义完整的 Dev_Lifecycle 六阶段流程及每个阶段的 Lifecycle_Gate 准入/准出标准。
2. WHEN 开发者在 Workspace_Root 下创建新的 Sub_Project 目录时，THE Steering_File SHALL 自动对该 Sub_Project 生效，Antigravity 无需额外加载配置。
3. THE Steering_File SHALL 规定需求分析阶段：Antigravity 必须先在 `harness-collab/01-product-specs/` 下创建需求文档，经确认后方可进入编码阶段。
4. THE Steering_File SHALL 规定技术设计阶段：Antigravity 必须在 `harness-collab/02-design-docs/` 下创建设计文档，包含架构图、接口定义和数据模型，经确认后方可生成代码。
5. THE Steering_File SHALL 规定编码实现阶段：Antigravity 生成的所有代码必须符合分层架构约束，并同步生成对应测试类。
6. THE Steering_File SHALL 规定测试验证阶段：Antigravity 必须确保新增代码的单元测试覆盖率不低于 80%，并在 `harness-collab/03-exec-plans/` 下记录测试执行结果。
7. THE Steering_File SHALL 规定文档同步阶段：Antigravity 在完成代码交付后必须同步更新 `harness-collab/04-api-docs/` 和 `harness-collab/func.md`。
8. THE Hook SHALL 在每个 Lifecycle_Gate 节点自动触发检查，若当前阶段产出物不满足准出标准，则阻止进入下一阶段并输出具体的修正指引。
9. THE Harness_Template SHALL 提供 `harness-collab/05-methodology/dev-workflow.md`，以流程图形式描述完整的 Dev_Lifecycle，供 Antigravity 和开发者随时参考。
