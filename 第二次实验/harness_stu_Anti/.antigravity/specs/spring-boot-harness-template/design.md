...
Spring Boot Harness 工程管理模板是一套部署在 Antigravity 工作区根目录的"监管层"基础设施。其核心思想是**规范即代码、门禁即流程**：通过 Antigravity 原生机制（steering 文件、hooks、spec 规范）将工程标准内嵌到 AI 辅助开发流程中，使 Antigravity 在每次对话和每次代码生成时都自动遵循团队约定，无需开发者手动重复说明约束。

### 核心定位

```
Workspace_Root/
├── .antigravity/                  ← Harness 监管层（本模板核心）
...
当开发者在 `Workspace_Root` 下创建任意 Spring Boot 子项目时，`.antigravity/steering/` 中的所有 steering 文件自动对该子项目生效，Antigravity 无需额外加载配置即可在整个 Dev_Lifecycle 六阶段中执行监管。
...
sequenceDiagram
    ...
    participant Antigravity as Antigravity IDE
    participant Steering as Steering 文件
    participant Hook as Hooks
    participant CI as GitHub Actions

    Dev->>Antigravity: 开始新功能开发
    Antigravity->>Steering: 加载 project-lifecycle.md
    
    Note over Dev,CI: 阶段1：需求分析
    Antigravity->>Dev: 要求在 harness-collab/01-product-specs/ 创建需求文档
    Dev->>Antigravity: 确认需求文档完成
    
    Note over Dev,CI: 阶段2：技术设计
    Antigravity->>Dev: 要求在 harness-collab/02-design-docs/ 创建设计文档
    Dev->>Antigravity: 确认设计文档完成
    
    Note over Dev,CI: 阶段3：编码实现
    Antigravity->>Steering: 检查分层架构约束
    Hook->>Dev: 新 Java 文件包路径检查
    
    Note over Dev,CI: 阶段4：测试验证
    Hook->>Dev: 提示同步更新测试文件
    Antigravity->>Dev: 确认覆盖率 ≥ 80%
...
Antigravity 在每次对话启动时自动扫描 `.antigravity/steering/` 目录，加载所有 `inclusion: auto` 的 steering 文件。这意味着无论开发者在工作区的哪个子目录下工作，这些约束都始终生效。
...
**职责**：约束 Antigravity 生成的 Java 代码符合分层架构和编码标准。
...
**职责**：定义 Antigravity 与开发者的协作协议，规范 AI 交付行为。

**关键约束内容**：
- Antigravity 在生成代码前必须确认需求文档和设计文档已存在
- Antigravity 不得跳过任何 Lifecycle_Gate 检查点
- Antigravity 在每次交付后必须输出交付摘要（修改文件列表、测试状态、文档同步状态）
...
**文件**：`.antigravity/hooks/api-doc-sync-check.md`
...
**文件**：`.antigravity/hooks/layer-constraint-check.md`
...
- 如路径不符合约定，输出警告：
  ...
  参考：.antigravity/steering/java-engineering-standards.md"
```

#### 2.3 测试覆盖提醒 Hook

**文件**：`.antigravity/hooks/test-coverage-reminder.md`
...
操作指引：参考 .antigravity/steering/testing-quality-standards.md"
```

#### 2.4 Maven Profile 继承提醒 Hook

**文件**：`.antigravity/hooks/maven-profile-check.md`
...
每个 Hook 文件（`.antigravity/hooks/*.md`）的结构：
...
| 场景 | 处理策略 |
|------|----------|
| Hook 文件格式错误 | Antigravity 跳过该 Hook，不影响其他 Hook 执行；在 IDE 日志中记录错误 |
...
3. 不强制阻断 Antigravity 继续工作，但在每次后续对话中重复提示直到问题解决
...
# Steering 文件
check_file ".antigravity/steering/java-engineering-standards.md"
check_file ".antigravity/steering/testing-quality-standards.md"
check_file ".antigravity/steering/api-doc-sync-protocol.md"
check_file ".antigravity/steering/ai-collaboration-protocol.md"
check_file ".antigravity/steering/project-lifecycle.md"

# Hooks
check_file ".antigravity/hooks/api-doc-sync-check.md"
check_file ".antigravity/hooks/layer-constraint-check.md"
check_file ".antigravity/hooks/test-coverage-reminder.md"
check_file ".antigravity/hooks/maven-profile-check.md"
...
```bash
grep -l "inclusion: auto" .antigravity/steering/*.md | wc -l
...
# 在 Antigravity 中打开 demo-project，验证 Antigravity 自动加载 steering 约束
# 要求 Antigravity 生成一个 Controller 类，验证其遵循分层架构约束
```
...
**步骤 5：Hook 触发验证（手动）**
```
在 Antigravity 中修改任意 controller 包下的 Java 文件
验证：Antigravity 自动输出 API 文档同步提示
```
