...
- [x] 1. 创建 Steering 文件体系
  - [x] 1.1 创建 `.antigravity/steering/java-engineering-standards.md`
  ...
  - [x] 1.2 创建 `.antigravity/steering/testing-quality-standards.md`
    - 包含 `inclusion: auto` 元数据头
  ...
  - [x] 1.3 创建 `.antigravity/steering/api-doc-sync-protocol.md`
    - 包含 `inclusion: auto` 元数据头
  ...
  - [x] 1.4 创建 `.antigravity/steering/ai-collaboration-protocol.md`
    - 包含 `inclusion: auto` 元数据头
    - Antigravity 生成代码前必须确认需求文档和设计文档已存在
    - 不得跳过任何 Lifecycle_Gate 检查点
    - 每次交付后必须输出交付摘要（修改文件列表、测试状态、文档同步状态）
    - _需求：9.3, 9.4, 9.5_

  - [x] 1.5 创建 `.antigravity/steering/project-lifecycle.md`
    - 包含 `inclusion: auto` 元数据头
...
- [x] 2. 创建 Hooks 自动化体系
  - [x] 2.1 创建 `.antigravity/hooks/api-doc-sync-check.md`
...
- [x] 5. 创建 harness-collab 文档体系
  ...
  - [x] 5.6 创建方法论文档
    ...
    - 创建 `harness-collab/05-methodology/ai-delivery-playbook.md`：AI 交付手册（Antigravity 操作规范、交付摘要格式）
    - _需求：6.6, 9.9_
...
- [-] 8. 检查点 - 运行验证脚本确认模板完整性
  ...
  - 执行 `grep -l "inclusion: auto" .antigravity/steering/*.md | wc -l`，确认输出为 5
  - 如有缺失文件，补充创建后重新验证
...
- 所有 steering 文件必须包含 `inclusion: auto` 元数据，确保 Antigravity 自动加载
