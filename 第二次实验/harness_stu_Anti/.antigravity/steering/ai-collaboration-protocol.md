...
本文件由 Antigravity 在每次对话中自动加载，定义 Antigravity 与开发者的协作规范，规范 AI 交付行为，对工作区内所有 Spring Boot 子项目强制生效。
...
**Antigravity 在生成任何业务代码前，必须确认以下条件已满足：**

1. `harness-collab/01-product-specs/` 下存在对应功能的需求文档
2. 需求文档已包含用户故事和验收标准
3. 开发者已明确确认需求文档内容

若上述条件未满足，Antigravity 必须：
- 拒绝生成业务代码
...
**Antigravity 在生成架构相关代码（Controller/Service/Repository 类、数据模型、配置类）前，必须确认：**
...
**Antigravity 不得跳过任何 Lifecycle_Gate 检查点。** 六个阶段的检查点如下：
...
**Antigravity 在生成业务代码时，必须同步生成对应的测试类。** 规则如下：
...
**Antigravity 每次代码交付后必须输出交付摘要**，格式如下：
...
当开发者要求 Antigravity 生成违反分层约束的代码时（例如：在 Controller 中直接注入 Repository），Antigravity 必须：
...
当开发者要求 Antigravity 跳过需求文档或设计文档检查时，Antigravity 必须：
